//! Persists a `kimi_k3::generate::KvState` across process restarts for `--chat --session <path>`
//! — the sibling of `kimi_linear::kv_session.rs`, ported nearly verbatim (same two-file
//! strategy, same crash-consistency rule) but for K3's own `Model`/`KvState`/`LayerState`/
//! `KdaLayerState` types (`kimi_k3::generate`'s own copies — see that module's doc for why
//! they're not a reuse of `kimi_linear::generate`'s) and K3's nested `Cfg` (fields read via
//! `model.cfg.base.*`, not flat).
//!
//! **Attention Residuals needs NO persistence here** — confirmed via `attn_res.rs`'s own doc:
//! `AttnResState` is created fresh inside `layers_forward`/`layers_forward_profiled` for every
//! single `step`/`step_all` call and discarded once that call returns (the real
//! `KimiLinearModel.forward` does the same — a brand-new empty `block_residual` every call).
//! There is no cross-step Attention-Residuals state to save, so this file's format is exactly
//! the same shape as Kimi Linear 48B's: one MLA log + one KDA snapshot, nothing more.
//!
//! Distinct magic bytes from `kimi_linear::kv_session.rs`'s own (`RBTK3MA1`/`RBTK3KD1` vs.
//! `RBTKVKM1`/`RBTKDAS1`) so a session file saved for one family is rejected outright (bad magic)
//! if ever pointed at the other, rather than misreading its bytes.
//!
//! All integers little-endian. See `kimi_linear::kv_session.rs`'s module doc for the full
//! two-file/crash-consistency reasoning — identical here, not repeated.

use crate::glm52::attention::KvCache;
use crate::kimi_k3::generate::{KdaLayerState, KvState, LayerState};
use crate::kimi_k3::model::{Attn, Model};
...
const MLA_MAGIC: &[u8; 8] = b"RBTK3MA1";
const MLA_VERSION: u32 = 1;
const KDA_MAGIC: &[u8; 8] = b"RBTK3KD1";
...
impl fmt::Display for LoadError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            LoadError::BadMagic => write!(f, "session file: bad magic (not a rabbit Kimi K3 KV session file)"),
...
/// `crate::kv_session::read_partial`'s doc (duplicated rather than shared, same "sibling, not
/// reused" relationship every `kv_session.rs` in this crate has to the others).
...
fn mla_shape_of_model(model: &Model) -> Vec<MlaShape> {
    let shape = MlaShape { kv_lora: model.cfg.base.kv_lora as u32, qk_rope: model.cfg.base.qk_rope as u32 };
    model.layers.iter().filter(|l| matches!(l.attn, Attn::Mla(_))).map(|_| shape).collect()
}

fn kda_shape_of_model(model: &Model) -> Vec<KdaShape> {
    let shape = KdaShape { n_heads: model.cfg.base.kda_n_heads as u32, head_dim: model.cfg.base.kda_head_dim as u32, kernel: model.cfg.base.short_conv_kernel as u32 };
...
fn save_mla_log(shape: &[MlaShape], state: &KvState, from_pos: usize, to_pos: usize, path: &Path) -> io::Result<()> {
    ...
    if exists {
        if from_pos == 0 {
            return Err(io::Error::new(io::ErrorKind::InvalidInput, "k3 kv_session: from_pos=0 but session file already exists"));
        }
        ...
        if file_shape != shape {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "k3 kv_session: session file shape does not match the current session"));
        }
    } else if from_pos != 0 {
        return Err(io::Error::new(io::ErrorKind::InvalidInput, "k3 kv_session: from_pos must be 0 when creating a new session file"));
    }
...
mod tests {
    use super::*;
    use crate::kimi_k3::generate::{step, ExpertCaches};
    ...
    /// One KDA layer (low-rank output gate) + one MLA layer (`q_lora=0`, no output gate), dense
    /// FFN, no Attention Residuals/latent-MoE — `kimi_k3::generate.rs`'s and `model.rs`'s own
    /// tests already cover K3-only-feature dispatch correctness in depth; this fixture exists
    /// only to give `save`/`load` real, non-trivial cross-family (KDA+MLA) state to persist.
    ...
    fn save_then_load_roundtrip_continues_identically() {
        let fixture = build_fixture("rabbit_test_k3_kvsession_roundtrip");
    ...
    fn two_turn_session_roundtrip_continues_identically() {
        let fixture = build_fixture("rabbit_test_k3_kvsession_two_turn");
        let model = Model::load(&fixture.0, 32, 32).unwrap();
        let shards = Shards::open(&fixture.0).unwrap();
        let session_path = fixture.0.join("session.bin");
    ...
    fn crash_after_mla_append_before_kda_replace_recovers_to_previous_turn() {
        let fixture = build_fixture("rabbit_test_k3_kvsession_crash_before_kda");
        let model = Model::load(&fixture.0, 32, 32).unwrap();
        let shards = Shards::open(&fixture.0).unwrap();
        let session_path = fixture.0.join("session.bin");
        ...
        // Advance a SEPARATE copy of the state through turn 2, but only append its MLA record --
        // deliberately skip the .kda replace, reproducing the crash window.
    ...
    fn truncated_trailing_mla_record_recovers_to_previous_complete_turn() {
        let fixture = build_fixture("rabbit_test_k3_kvsession_truncated");
        let model = Model::load(&fixture.0, 32, 32).unwrap();
        let shards = Shards::open(&fixture.0).unwrap();
        let session_path = fixture.0.join("session.bin");
    ...
    fn kda_shape_mismatch_on_load_is_a_hard_error() {
        let fixture = build_fixture("rabbit_test_k3_kvsession_shape_mismatch");
        let model = Model::load(&fixture.0, 32, 32).unwrap();
        let shards = Shards::open(&fixture.0).unwrap();
        let session_path = fixture.0.join("session.bin");
        ...
        model_mismatched.cfg.base.kda_head_dim = 8;
    ...
    #[test]
    fn wrong_family_magic_is_rejected() {
        // A Kimi Linear 48B session file (different magic bytes) pointed at a K3 model must be
        // rejected outright, not misread.
        let fixture = build_fixture("rabbit_test_k3_kvsession_wrong_magic");
        let model = Model::load(&fixture.0, 32, 32).unwrap();
        let session_path = fixture.0.join("session.bin");
        fs::write(&session_path, b"RBTKVKM1\x01\x00\x00\x00\x00\x00\x00\x00").unwrap(); // kimi_linear's own magic
        fs::write(kda_path(&session_path), b"RBTKDAS1\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00").unwrap();

        match KvState::load(&session_path, &model) {
            Err(LoadError::BadMagic) => {}
            Err(e) => panic!("expected BadMagic, got a different error: {e}"),
